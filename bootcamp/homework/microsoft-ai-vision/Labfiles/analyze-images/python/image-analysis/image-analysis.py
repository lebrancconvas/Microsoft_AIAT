...
def main():
    ...
    try:
        ...
        # Get image
        image_file = './images/uncle_roger.jpg'     
        if len(sys.argv) > 1:
        ...
        if result.caption is not None:
            print("\nCaption:")
            print(f" '{result.caption.text}' (confidence: {result.caption.confidence * 100:.2f}%)")
        ...
        if result.objects is not None:
            ...
            for obj in result.objects.list:
                print(f" - {obj.tags[0].name} (confidence: {obj.tags[0].confidence * 100:.2f}%)")
        ...
        if result.people is not None:
            ...
            for person in result.people.list:
                print(f" - Bounding box {person.bounding_box}, Confidence: {person.confidence * 100:.2f}%")
...
def show_people(image_filename, detected_people):
    ...
    for detected_person in jpgdetected_people:
